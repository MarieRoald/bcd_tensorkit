...
if __name__ == "__main__":
    logs, summaries = load_summaries(LOG_FOLDER, NUM_EXPERIMENTS)        
    summary = make_summary_df(summaries)
    print(summary.groupby(level=0).median())
    make_log_time_plot(logs, "FMS")
    plt.figure()
    make_log_plot(logs, "FMS",)
    plt.show()
